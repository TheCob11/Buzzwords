...
class NNet:
    ...
    def __init__(self, layerSizes: list[int], layerActivators=[Activators.reLU,Activators.sigmoid]):
    ...
    def backProp(self, input, expected, logNumbers=False):
        ...
        dCdz = self.costPrime(
            activations[-1], np.transpose([expected])) * self.layerActivators[-1].prime(preActivations[-1])
        if(logNumbers):
            print(activations[-1])
            print("\n")
            print(expected)
    ...
    def costPrime(self, output, expected):
...
if __name__ == "__main__":
    n = NNet([2, 3, 1])
    n.train(XOR.generateData(100), 1, 10, 40)
